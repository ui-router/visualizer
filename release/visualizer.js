(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["ui-router-visualizer"] = factory(require("angular"));
	else
		root["ui-router-visualizer"] = factory(root["angular"]);
...
/***/ function(module, exports, __webpack_require__) {
	...
	"use strict";
	exports.moduleName = "ui.router.statevis";
	    ...
	    return {
	        ...
	        controller: function ($scope, $element) {
	            ...
	            $transitions.onBefore({}, function ($transition$) {
	                ...
	                $transition$.promise.finally(function () { return delete $transition$._message; });
	            });
	...
	statevis_module_1.app.directive('uirTransitionView', function () {
	    ...
	    return {
	        ...
	        controller: function () {
	            ...
	            this.fullScreen = function (toggle) { return _this.toggles.fullscreen = toggle; };
	            // Provides the icon class to the view
	            this.iconClass = function () { return iconClasses[_this.status]; };
	            this.tc = this.trans.treeChanges();
	            var paths = angular.extend({}, this.tc);
	            // Ignore the root state when drawing paths.
	            ["entering", "exiting", "retained"].forEach(function (key) { return paths[key] = paths[key].filter(function (node) { return node.state.name; }); });
	            ...
	            var paramsForNode = function (node) {
	                return Object.keys(node.values).map(function (key) { return ({ state: node.state.name, key: key, value: node.values[key] }); });
	            };
	            ...
	            this.paramsMap = this.params.reduce((function (obj, param) { obj[param.key] = param.value; return obj; }), {});
	            var success = function () { return _this.status = "success"; };
	            var reject = function (rejection) {
	                _this.status = "error";
	                if (rejection) {
	                    _this.rejection = rejection && rejection.message;
	                    var type = rejection && rejection.type;
	                    if (type == 2) {
	                        _this.status = "redirected";
	                        //this.rejection = rejection.detail;
	                        var toState = rejection.detail.name();
	                        var toParams = JSON.stringify(rejection.detail.params());
	                        _this.rejection = truncateTo(100, toState + "(" + toParams) + ")";
	                    }
	                    if (type == 5) {
	                        _this.status = "ignored";
	                        _this.rejection = "All states and parameters in the To and From paths are identical.";
	                    }
	...
	statevis_module_1.app.directive('uirTransitionNodeDetail', function () { return ({
	    ...
	    controller: function ($scope) {
	        ...
	        $scope.$watch(function () { return _this.node; }, function (node, oldval) {
	            if (!node)
	                return;
	            _this.params = node.schema.reduce(function (params, param) {
	                params[param.id] = node.values[param.id];
	                return params;
	            }, {});
	        });
	        var getResolveKeys = memoDebounce(500, function () {
	            return Object.keys(_this.node && _this.node.resolves || {}).filter(function (key) { return key !== '$stateParams' && key !== '$transition$'; });
	        });
	        this.unwrapResolve = function (resolve) {
	            return resolve.data;
	        };
	        $scope.$watchCollection(getResolveKeys, function (keys, oldval) {
	            _this.resolves = (keys || []).reduce((function (resolves, key) {
	                resolves[key] = _this.node.resolves[key];
	                return resolves;
	            }), {});
	        });
	    },
